...
function schemaToType(schema: any): string {
  ...
  switch (shapeType) {
    ...
    case undefined: {
      if (!dataImported) {
        imports += `import { type PlutusData } from "@blaze-cardano/core";`;
        dataImported = true;
      }
      return "PlutusData";
    }
...
export type BlueprintArgs = { infile?: string; outfile?: string };
export function generateBlueprint({
    ...
    function (${paramsArgs.map((param) => param.join(":")).join(",")}) {${
      paramsArgs.length > 0
        ? `return cborToScript(applyParamsToScript("${script}", [${paramsArgs
            ...
            .join(
              ",",
            )}], ${JSON.stringify(paramsSchema)} as any), ${plutusVersion});`
  ...
  fs.writeFile(outfile, plutus, (err) => {
    if (err) {
      console.error(err);
    }
  });

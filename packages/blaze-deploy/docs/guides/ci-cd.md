...
jobs:
  deploy:
    ...
    steps:
      ...
      - run: bun --filter @blaze-cardano/deploy build
      ...
      - uses: actions/upload-artifact@v4
        with:
          ...
          path: examples/script-deploy-ci/deployment-cache.json
```
...
A deployment job should fail if the provider cannot resolve the script reference after confirmation. That usually means the submitted transaction and the provider-visible chain state disagree, or the provider has not indexed the script yet. Retry after provider indexing instead of writing an unverified cache record.

## Release evidence

For a public release, attach the manifest source, deployment cache snapshot, CI run URL, transaction IDs for new deployments, package version, and docs URL. The cache snapshot includes the resolved script-reference UTxO CBOR, so another developer can replay the intended deployment state and compare it with live chain data.

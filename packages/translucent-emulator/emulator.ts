...
import { TransactionUnspentOutput, ProtocolParameters, TransactionOutput, Transaction, TransactionId, TransactionInput, RewardAccount, StakeRegistration, Address, AddressType } from "../translucent-core";
...
export class Emulator {
     ...
     */
    constructor(genesisOutputs: TransactionOutput[], params: ProtocolParameters = hardCodedProtocolParams){
        for (let i=0; i < genesisOutputs.length; i++){
            let txIn = new TransactionInput(TransactionId("00".repeat(32)), BigInt(i))
    ...
    private acceptTransaction(tx: Transaction) {
        ...
        for (let i = 0; i < outputs.length; i++){
            const utxo = new TransactionUnspentOutput(new TransactionInput(txId, BigInt(i)), outputs[i])
        ...
        const withdrawals: Map<RewardAccount, bigint> = tx.body().withdrawals() ?? new Map()

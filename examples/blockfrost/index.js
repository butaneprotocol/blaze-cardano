...
const txIn = new TransactionInput(
  "6b1a46020a7864749e875f69e904c820cadc334147de0bfacafbeaa89d1ac7e9",
  2,
);
const txOut = new TransactionOutput(
  "addr_test1qp9zjnc775anpndl0jh3w7vyy25syfezf70d",
  new Value(42338198n),
);
const offchainUtxo = new TransactionUnspentOutput(txIn, txOut);
...
const additionalUtxos = await provider.getUnspentOutputs(
  Core.addressFromBech32(
    "addr_test1qqpe6v4c95v0jlgjm5578w3san3eycf0hcfxwpp0yafze8mxjxnr4tyryejp4kxyqu0u82wv4hx6f6pqns9dknetaqgsmzzz6e",
  ),
);

const redeemersWithUtxos = await provider.evaluateTransaction(
  evalTx,
  additionalUtxos,
);

console.log("\nEvaluate Tx with UTxOs:");
for (const redeemer of redeemersWithUtxos.values()) {
  console.log(`Memory - ${redeemer.exUnits().mem()}`);
  console.log(`CPU - ${redeemer.exUnits().steps()}`);
}
